...
export function resolveObsidianConfig(
  ...
  const defaultProject = env.DEVGOD_OBSIDIAN_DEFAULT_PROJECT?.trim() || env.DEVGOD_PROJECT_NAME?.trim() || options.projectSlug;
  const timezone = validateTimezone(env.DEVGOD_OBSIDIAN_TIMEZONE?.trim() || "Europe/Madrid");
  const vaultPath = env.DEVGOD_OBSIDIAN_VAULT_PATH?.trim();
...
export async function validateObsidianConfig(config: ObsidianExportConfig): Promise<void> {
  ...
  if (!config.vaultPath) {
    throw new Error("Obsidian vault path is not configured. Set DEVGOD_OBSIDIAN_VAULT_PATH first.");
  }
