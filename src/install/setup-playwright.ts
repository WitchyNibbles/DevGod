...
function npxCommand(): string {
  if (typeof process.env.DEVGOD_PLAYWRIGHT_NPX_BIN === "string" && process.env.DEVGOD_PLAYWRIGHT_NPX_BIN.length > 0) {
    return process.env.DEVGOD_PLAYWRIGHT_NPX_BIN;
  }
...
async function installChromium(repoRoot: string): Promise<void> {
  ...
  if (process.platform === "linux" && (process.env.CI === "true" || process.env.DEVGOD_PLAYWRIGHT_INSTALL_DEPS === "1")) {
    args.push("--with-deps");
  }
  args.push("chromium");
