...
In Codex, this registry is not a standalone starting point. Substantive UI tasks should route through `devgod-ui-art-direction` first, declare the `ui surface`, then pull from this registry as part of the chosen frontend skill chain.
...
- strong enough to support interaction choreography, imagery strategy, and immersive menu systems rather than static shells only
...
Why it ships:
- public browse surface exposes thousands of examples across categories, colors, platforms, and years
- explicitly offers full-page screenshots and video recordings
- especially strong for scanning page density, section sequencing, and conversion-oriented composition
...
Useful, but the free plan explicitly limits access to categories, sections, and filter/search results. That makes it weaker as a required autonomous workflow dependency under the public/free constraint.

## Required usage pattern

For substantive redesign work:

1. declare the `ui surface` and use `devgod-ui-art-direction` as the frontend entrypoint before collecting references
...
11. record the likely implementation stack for the chosen direction, for example `Motion`, `Rive`, `GSAP`, `three.js`, CSS, video, or hybrid media
...
13. add official implementation guidance to the plan when immersive motion is central:
   ...
   - `rive.app/docs/runtimes` and `rive.app/docs/editor/state-machine/state-machine` for authored interactive motion graphics
...
22. when cute, magical, or lively atmosphere is requested, define a `semantic charm map` showing where stars, familiars, hats, candles, cauldrons, floating code, particles, or other motifs belong and what job they serve
...
- opening multiple frontend guidance files with no declared UI surface or routing entrypoint
