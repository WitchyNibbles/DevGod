...
These are the shared `devgod` defaults for branch names, commit subjects, and PR metadata.
...
For deterministic local branch-hook overrides, add a line to the consuming repo `AGENTS.md`:
...
- do not use `codex` in PR titles or PR bodies
