...
if [[ -z "$branch_name" ]]; then
  echo "devgod branch guard: work on a named branch created from updated origin/main" >&2
  exit 1
fi

if [[ "$normalized_branch_name" =~ (^|/)codex($|[-/]) ]]; then
  echo "devgod branch guard: do not use 'codex' in branch names" >&2
  exit 1
fi
...
printf 'devgod branch guard: branch "%s" must start with one of: %s\n' \
  "$branch_name" \
