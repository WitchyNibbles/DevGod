...
2. Put tokens near the implementation surface that owns them; avoid burying them in prose-only docs.
...
6. Verify the system on at least:
   - one dense screen
   - one empty or sparse state
   - one mobile layout
...
Return:
- token decisions introduced or reused
- components or surfaces normalized
- remaining system inconsistencies
