...
test("MemoryStore.getProjectRuntimeState returns defensive copies for runtime integrity state", async () => {
  const store = new MemoryStore();
  const runtimeState = {
    ...
    metadata: {
      ...
      lastIntegrityRepair: {
        ...
        summary: "cleared stale seed failure metadata",
  ...
  const firstRead = await store.getProjectRuntimeState(runtimeState.projectId);
  assert.ok(firstRead);
  firstRead.activeTaskId = "task-mutated";
  firstRead.taskQueue.current_task_id = "task-mutated";
  firstRead.taskQueue.tasks[0]!.status = "done";
  (firstRead.metadata!.seedFailure as { reason: string }).reason = "mutated failure";

  const secondRead = await store.getProjectRuntimeState(runtimeState.projectId);
  assert.ok(secondRead);
  assert.equal(secondRead.activeTaskId, "task-1");
  assert.equal(secondRead.taskQueue.current_task_id, "task-1");
  assert.equal(secondRead.taskQueue.tasks[0]!.status, "in_progress");
  assert.equal((secondRead.metadata!.seedFailure as { reason: string }).reason, "synthetic failure");
});

test("failTask releases active locks and persists runtime seed failure metadata", async () => {
  const store = new MemoryStore();
  const { service } = createService(store);
  const run = await service.intakeRequest({
    workspaceSlug: "team",
    projectSlug: "devgod",
    actor: "ceo",
    title: "Fail active task safely",
    request: "Ensure runtime failure leaves explicit residue but no stranded lock."
  });
  ...
  await service.claimTask(run.id, "task-fail", "planner");
  ...
  await store.saveProjectRuntimeState({
    ...
    productState: { currentTask: "task-fail", blockers: [] },
  ...
  await service.failTask(run.id, "task-fail", "synthetic runtime seed failure");

  const task = await store.getTask(run.id, "task-fail");
  assert.equal(task?.status, "blocked");
  assert.equal(task?.claimedBy, undefined);

  const locks = await store.getActiveLocks(projectContext.project.id);
  ...
  assert.equal(runtimeState.activeRunId, run.id);
  assert.equal(runtimeState.taskQueue.project_status, "ready");
  assert.equal(runtimeState.taskQueue.current_task_id, null);
  assert.equal(runtimeState.taskQueue.tasks[0]?.status, "blocked");
  assert.equal(runtimeState.taskQueue.tasks[0]?.blocker, "runtime task blocked");
  assert.equal((runtimeState.metadata?.seedFailure as { taskId?: string }).taskId, "task-fail");
  assert.match(
